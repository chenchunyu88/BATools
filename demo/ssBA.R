...
init=set_init("driploss",data=PigPheno,geno=genoNew,"id",df=5,pi_snp=1,h2=0.5,c=NULL,model="ssBayesA",centered=TRUE)
...
op<-create.options(model="ssBayesA",method="MCMC",priors=NULL,init=init,
                   update_para=update_para,run_para=run_para,save.at="ssBayesA",print_mcmc=print_mcmc)

ssBA<-baFit(driploss~sex,data=PigPheno,geno=genoNew ,genoid = ~id,options = op,map=PigMap,GWA="Win",PedAinv = PigAinv)
ssBA
par(mfrow=c(1,2))
man_plot_prob(ssBA)
...
cvssBA<-baFit(driploss~sex,data=PigPheno,geno=genoNew ,genoid = ~id,options = op, train=~cv1,PedAinv = PigAinv)
cvssBA
par(mfrow=c(1,1))
plot(cvssBA)
