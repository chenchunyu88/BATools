...
init=set.init(~driploss,data=PigPheno,geno=geno,~id,df=5,pi_snp=0.001,h2=0.5,c=NULL,model="BayesB",centered=TRUE)
run_para=list(niter=2000,burnIn=1000,skip=10)
print_mcmc=list(piter=500)
update_para=list(df=FALSE,scale=TRUE,pi=F)
op<-set.options(model="BayesB",method="MCMC",priors=NULL,init=init,
